...
# Setup SPDK on a VM
# Needs some cleanup - too much output.

echo ""
echo "###--- Running spdk.sh ---###"
echo ""

###--- Grab SPDK bundle from Github
git clone https://github.com/spdk/spdk /root
cd /root/spdk
...
if [ -s /etc/redhat-release ]; then
	...
	yum install -y gcc gcc-c++ make CUnit-devel libaio-devel openssl-devel git astyle-devel python-pep8 lcov python clang-analyzer libuuid-devel sg3_utils libiscsi-devel
...
###--- Compile SPDK (can't use RDMA so don't add that feature)
./configure && make
scripts/setup.sh
